  ...
  if (sock.m_congestionControl)
    {
      ...
      // check if santa cruz then enable option
      if (m_congestionControl->GetName() == "TcpSantaCruz")
      {
        m_sackEnabled = false;
        m_san_cruz_enabled = true;
      }
      else m_san_cruz_enabled = false;
    }
  ...
  if (tcpHeader.GetFlags () & TcpHeader::SYN)
    {
      ...
      if (tcpHeader.GetFlags () & TcpHeader::ACK)
        {
          if (m_san_cruz_enabled)
          {
            if(!tcpHeader.HasOption(TcpOption::SANTACRUZ))
            {
              // Ignoring segment without SANTACRUZ
                  NS_LOG_LOGIC ("At state " << TcpStateName[m_state] <<
                                " received packet of seq [" << seq <<
                                ":" << seq + packet->GetSize () <<
                                ") without SANTACRUZ option. Silently discard it");
                  return;
            }
            else
            {
                ProcessOptionSantaCruz (tcpHeader.GetOption (TcpOption::SANTACRUZ));
            }
          }
...
TcpSocketBase::EnterRecovery (uint32_t currentDelivered)
{
  ...
  //NS_LOG_UNCOND ("Entered Recovery");
      ...
      // of RecoveryPoint.
      ...
      else
        {
          ...
          if (exitedFastRecovery)
            {
              //NS_LOG_UNCOND("Exiting recovery");
...
TcpSocketBase::SendEmptyPacket (uint8_t flags)
{
  ...
  if (flags & TcpHeader::ACK)
    { // If sending an ACK, cancel the delay ACK as well
      ...
      //sending pure ack set santa cruz option
      if(m_san_cruz_enabled)
      {
        AddOptionSantaCruz(header, false, true);
      }
...
TcpSocketBase::SendDataPacket (SequenceNumber32 seq, uint32_t maxSize, bool withAck)
{
  ...
  if(m_san_cruz_enabled)
  {
    if(withAck) AddOptionSantaCruz(header, true, true);
    else AddOptionSantaCruz(header, true, false);
  }
...
void
TcpSocketBase::ProcessOptionSantaCruz (const Ptr<const TcpOption> option)
{
  NS_LOG_FUNCTION (this << option);

  //Processing option santa cruz

  Ptr<const TcpOptionSantaCruz> sc = DynamicCast<const TcpOptionSantaCruz> (option);

  uint16_t packet_id = sc->GetPacketIdAck();
  uint32_t receivetime = sc->GetTimestamp();

  if(sc->GetPacketIdData() == -1)
  {
    //Not an data packet
  }
  else
  {
    last_data_pkt_rcvd = sc->GetPacketIdData();
  }
  ...
  int sz = packet_sendtime_hist.size();
  for(int i = 0; i < sz; i++)
  {
    packet_sendtime_pair temp = packet_sendtime_hist[i];
    if(temp.first == packet_id)
    {
      sendtime = temp.second;
      found = true;
      break;
    }
  }

    if(is_first_ack_for_data)
    {
      //Processing option santa cruz first ack
      is_first_ack_for_data = false;
      if(found)
      {
        first_packet_sendtime_pair = std::make_pair(packet_id,sendtime);
        first_packet_receivetime_pair = std::make_pair(packet_id, receivetime);
        relative_delays.push_back(0);
      }
    }
    else
    {
      if(found)
      {
        //Processing option santa cruz doing calculation
        uint16_t prev_pkt_id = first_packet_sendtime_pair.first;

        uint32_t send_interval = sendtime - first_packet_sendtime_pair.second;
        uint32_t receive_interval = receivetime = first_packet_receivetime_pair.second;
        first_packet_sendtime_pair = std::make_pair(packet_id,sendtime);
        first_packet_receivetime_pair = std::make_pair(packet_id, receivetime);

        uint32_t forward_Delay_change = receive_interval-send_interval;
        relative_delays.push_back(forward_Delay_change);
        packet_receivetime_hist.push_back(std::make_pair(packet_id,receivetime));

        double pktS = std::abs(double(receive_interval/(packet_id-prev_pkt_id)));

        uint32_t delay_sum = relative_delays[relative_delays.size()-1]+relative_delays[relative_delays.size()-2];
        
        double current_queueing_introduced = delay_sum/pktS;

        double current_queued_packets = prev_queued_pkts + current_queueing_introduced;

        //std::cout<<"queued = "<<current_queued_packets<<std::endl;

        if(current_queued_packets < (operating_point-delta) && current_queued_packets>0)
        {
          //increase congestion window
          m_congestionControl->IncreaseWindowSantaCruz(m_tcb, current_queued_packets, operating_point);
        }
        else if(current_queued_packets > (operating_point+delta))
        {
          //decrease congestion window
          m_congestionControl->DecreaseWindowSantaCruz(m_tcb, current_queued_packets, operating_point);
        }
        prev_queued_pkts = current_queued_packets;
      }
    }
  }
  // NS_LOG_INFO (m_node->GetId () << " Got timestamp=" <<
...
void
TcpSocketBase::AddOptionSantaCruz (TcpHeader& header, bool withData, bool withAck)
{
  NS_LOG_FUNCTION (this << header);

  //NS_LOG_UNCOND("Adding option santa cruz");

  Ptr<TcpOptionSantaCruz> option = CreateObject<TcpOptionSantaCruz> ();

  option->SetTimestamp (TcpOptionSantaCruz::NowToTimeStampValue ());
  if(withData)
  {
    option->SetPacketIdData(packet_id_data);
    packet_id_data++;
  }
  else option->SetPacketIdData(-1);

  if(withAck)
  {
    option->SetPacketIdAck(last_data_pkt_rcvd);
  }
  else option->SetPacketIdAck(-1);

  header.AppendOption (option);
  //NS_LOG_UNCOND("Exiting Adding option santa cruz");
  //NS_LOG_INFO (m_node->GetId () << " Add option Santa Cruz, time= " <<
              // option->GetTimestamp () << " data packet id = " << option->GetPacketIdData()<<" ack packet id= "<<option->GetPacketIdAck());
}
...
TcpSocketBase::SetCongestionControlAlgorithm (Ptr<TcpCongestionOps> algo)
{
  ...
  //check if santa cruz if yes, enable option

  if (algo->GetName() == "TcpSantaCruz")
  {
    //m_sackEnabled = false;
    m_san_cruz_enabled = true;
  }

...
protected:
  ...
  //santa cruz parameters
  bool m_san_cruz_enabled       {true};
  uint16_t packet_id_data       {1}; //packet id from sender send this to receiver so he can send ack for this
  uint16_t last_data_pkt_rcvd   {0}; //packet_id_from sender send ack for this
  bool is_first_ack_for_data    {true};
  //bool sending_ack              {false};

  typedef std::pair<uint16_t, uint32_t> packet_sendtime_pair;
  std::vector<packet_sendtime_pair> packet_sendtime_hist;
  std::vector<packet_sendtime_pair> packet_receivetime_hist;
  packet_sendtime_pair first_packet_sendtime_pair;
  packet_sendtime_pair first_packet_receivetime_pair;
  std::vector<uint32_t> relative_delays;

  uint32_t operating_point = 4;
  uint32_t delta = 1;
  double prev_queued_pkts = 0;


  /** \brief Process the santa cruz option from other side
   *
   * get the timestamp, calculate forward delay, add to the delay sum
   * replace the current packet info in the first pair 
   * do other calculations too? 
   *
   * \see EstimateRtt
   * \param option Option from the segment
   */
  void ProcessOptionSantaCruz (const Ptr<const TcpOption> option);
  /**
   * \brief Add the santacruz option to the header
   *
   * set the data packet id if data is being sent
   * set the ack id if ack is sent and arrival time of data
   *
   * \param header TcpHeader to which add the option to
   */
  void AddOptionSantaCruz (TcpHeader& header, bool withData, bool withAck);
